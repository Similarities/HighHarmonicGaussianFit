...
class GaussianFitHighHarmonicDivergence:
    ...
    def background_x(self):
        back_mean = np.mean(self.picture[1990:2048, :], axis=0)
        for x in range(0, 2048):
            self.picture_background[x, ::] = self.picture[x, ::] - back_mean[x]
        return self.picture_background
    ...
    def nm_in_px(self, energy_nm):
        ...
        self.px_boarder = int(4.71439193e-01 * energy_nm ** 2 - 1.06651902e+02 * energy_nm + 4.29603367e+03)
        return self.px_boarder

    def energy_range(self):
        # rint(self.harmonic_selected)
        ...
        # rint(previous_harmonic, next_harmonic)
    ...
    def print_h_lines(self):
        ...
        #plt.hlines(self.fundamental_in_px, xmin=0, xmax=2048, color='r', linewidth=0.1)
    ...
    def prepare_header(self):
        ...
        return np.vstack((header_names, parameter_info, self.gaussian_result))

    def save_data(self):
        result = self.prepare_header()
        print('saved data')
        np.savetxt(self.filedescription[23:29] + '_' + self.filedescription[-6:-4] + ".txt", result, delimiter=' ',
                   header='string', comments='',
                   fmt='%s')

    def file_description(self):
        return self.filedescription[-6:-4]

    def file_header(self):
        return self.filedescription[23:29]
...
def plot_picture(array, label_name, symbol):
    plt.figure(100)
    plt.scatter(array[::, 0], array[::, 1], label=label_name,
                marker=symbol)
    plt.xlabel('harmonic number [N]')
    plt.ylabel('sigma (1/e) divergence [mrad]')
    plt.legend()

def process_files(my_files, path, my_fundamental):
    symbols = ("x", ".", "s", "*", "v", "+", '.', '.','.', '.', '.', '.', '.', '.','.', '.','.', '.','.', '.','.', '.', '.', '.', '.','.', '.','.', '.','.', '.')
    for x in range(0, len(my_files)):
        file = path + '/' + my_files[x]
        ProcessImage = GaussianFitHighHarmonicDivergence(file, my_fundamental[x],
                                                         17)
        ProcessImage.open_file()
        ProcessImage.background()
        my_gaussian_fit = ProcessImage.batch_over_N()
        label_name = str(ProcessImage.file_header()) + str(ProcessImage.file_description())

        plot_picture(my_gaussian_fit, label_name, symbols[x])
...
# insert the following ('filepath/picture_name.tif', fundamental frequency (float), harmonic number (int, first harmonic),
# "picture name for plot")

# Todo  plot with description from file-name div(gauss)/N best: rad logarithm
# sort for z distance
# Todo change central frequency !


path_picture = '20190123_selection/3600'
my_files = get_file_list(path_picture)
my_fundamental = (798, 798, 798, 803, 800, 804, 800, 800, 800, 800, 800, 800, 800, 800, 800,800, 800, 800,800, 800, 800, 800, 800, 800,800, 800, 800)

process_files(my_files, path_picture, my_fundamental)
plt.show()
